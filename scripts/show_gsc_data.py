...
def show_gsc_data():
    ...
    try:
        ...
        # Get search queries data
        logger.info("Top 10 queries by clicks:")
        queries = collector.collect_search_queries(start_date, end_date)
        if queries:
        ...
        pages = collector.collect_pages(start_date, end_date)
        if pages:
        ...
        position_metrics = collector.collect_position_metrics(start_date, end_date)
        if position_metrics:
